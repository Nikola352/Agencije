            ...
            <Link to={`/destinacija/${gid}/${dest.id}`} className="h-full w-full">
                <figure className='relative w-full h-3/4 overflow-hidden'>
                    <img src={dest.thumbnail} alt={dest.naziv} className="relative w-full h-full object-cover" />
                        <h2 
                            dangerouslySetInnerHTML={{ __html: dest.naziv }}
                            className='absolute left-0 bottom-0 m-4 text-3xl sm:text-4xl font-bold text-white group-hover:text-primary-500 shadow-black [text-shadow:_0_1px_0_var(--tw-shadow-color)] group-hover:[text-shadow:_0_2px_0_var(--tw-shadow-color)]'
                        ></h2>
                </figure>     
